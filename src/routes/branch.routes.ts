...
branchRouter.get('/:companyId/:branchId/patients', asyncHandler(authorization([Roles.SUPERADMIN, Roles.ADMINISTRATOR, Roles.ADMIN])), asyncHandler(branchController.getAllBranchPatients));
branchRouter.get('/:companyId/:branchId/admin', asyncHandler(authorization([Roles.SUPERADMIN, Roles.ADMINISTRATOR])), asyncHandler(branchController.getAllBranchAdmins));
